...
export function DiscoveryModeScreen() {
  ...
  const persistDiscoveryMode = async (mode: "voice" | "text" | "guided") => {
    if (!sessionId) {
      return;
    }

    try {
      await updateSession(sessionId, { discovery_mode: mode });
    } catch (error) {
      console.error("Failed to persist discovery mode:", error);
    }
  };
  ...
  const handleVoice = async () => {
    ...
    await persistDiscoveryMode("voice");
  ...
  const handleText = async () => {
    ...
    await persistDiscoveryMode("text");
    navigate("/voice-discovery");
  };
  ...
  const handleQuestions = async () => {
    ...
    await persistDiscoveryMode("guided");
    navigate("/questions");
  };
