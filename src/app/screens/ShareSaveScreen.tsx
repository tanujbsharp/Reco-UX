...
export function ShareSaveScreen() {
          ...
          {activeProducts.map(p => (
            <div key={p.id} className="rounded-2xl border border-blue-100 bg-white/70 p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge className="rounded-full bg-[#2563eb] px-3 py-1 text-white">{p.matchScore}% match</Badge>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <TwoZoneLayout
      ...
      backHref={selectedProducts.length === 2 ? "/comparison" : `/product/${selectedProductId || activeProducts[0]?.id || ""}`}
